...
public class EmprestimoDAO {
    ...
    public boolean devolverLivro(int idEmprestimo, int livroId) {

        String sqlEmprestimo = "UPDATE emprestimos SET status = 'DEVOLVIDO' WHERE id = ?";
        String sqlLivro = "UPDATE livros SET disponibilidade = 'DISPONIVEL' WHERE id = ?";

        try (Connection conn = ConexaoMySQL.conectar()) {
            conn.setAutoCommit(false);

            try (PreparedStatement stmt1 = conn.prepareStatement(sqlEmprestimo);
